...
namespace FOS.App.Supervisor.Mappers
{
    public static class StudentAcademicMapper
    {
        public static AcademicYearDTO ToDTO(this AcademicYear academicYear,
            string programName,
            List<Supervisor.DTOs.StudentCoursesDTO> courses,
            decimal? cGpa,
            int? cHours,
            bool finalRecord)
        {
            try
            {
                ...
                {
                    academicYearDTO.SGPA = GpaIncludedCourses.Count() > 0 ?
                ...
                {
                    academicYearDTO.CGPA = System.Math.Round((decimal)(((academicYearDTO.SGPA * semesterIncludedHours) + (cGpa * cHours)) / (cHours + semesterIncludedHours)), 4);
                    academicYearDTO.CHours = semesterIncludedHours + cHours;
                    academicYearDTO.SGPA = System.Math.Round((decimal)academicYearDTO.SGPA, 4);
                }
                else
                {
                    if (!finalRecord)
                    {
                        academicYearDTO.CGPA = System.Math.Round((decimal)cGpa, 4);
